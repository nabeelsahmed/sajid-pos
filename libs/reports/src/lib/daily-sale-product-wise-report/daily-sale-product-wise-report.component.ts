...
@Component({
  selector: 'aims-pos-daily-sale-product-wise-report',
  templateUrl: './daily-sale-product-wise-report.component.html',
  styleUrls: ['./daily-sale-product-wise-report.component.scss'],
})
export class DailySaleProductWiseReportComponent implements OnInit {
  cmbOutlet: string = '';
  lblPartyName = '';
  dtpFromDate = '';
  dtpToDate = '';
  ...
  showReport() {
    ...
    this.dataService
      .getHttp(
        'report-api/PosCoreReport/getDailySalesOutletProductWise' + params,
        ''
      )
      .subscribe(
        (response: any) => {
  ...
  getDailySale(params: any) {
    this.dataService
      .getHttp('report-api/PosCoreReport/getDailySalesProductWise' + params, '')
      .subscribe(
        (response: any) => {
          for (var i = 0; i < response.length; i++) {
            var found = false;
            for (var j = 0; j < this.reportList.length; j++) {
              if (response[i].productID === this.reportList[j].productID) {
                found = true;
                this.reportList[j].qty =
                  parseFloat(this.reportList[j].qty) +
                  parseFloat(response[i].qty);
                this.reportList[j].salePrice =
                  parseFloat(this.reportList[j].salePrice) +
                  parseFloat(response[i].salePrice);
  ...
  exportExcel() {
    this.globalService.exportExcel('section', 'Daily Sale Product Wise');
  }
}

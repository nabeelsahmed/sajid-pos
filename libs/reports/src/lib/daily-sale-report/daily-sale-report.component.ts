...
export class DailySaleReportComponent implements OnInit {
  ...
  showReport() {
    ...
    this.dataService
      .getHttp('report-api/PosCoreReport/getDailySalesOutlet' + params, '')
      .subscribe(
        (response: any) => {
          ...
          for (var i = 0; i < response.length; i++) {
            ...
            this.lblTotalSalePrice +=
              parseFloat(response[i].salePrice) * parseFloat(response[i].qty);
          }
  ...
  getDailySale(params: any) {
    this.dataService
      .getHttp('report-api/PosCoreReport/getDailySales' + params, '')
      .subscribe(
        (response: any) => {
          // this.lblTotalQty = 0.0;
          // this.lblTotalSalePrice = 0.0;
  ...
  exportExcel() {
    this.globalService.exportExcel('section', 'Daily Sale');
  }
}

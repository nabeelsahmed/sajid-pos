...
export class PortalPriceConfigComponent implements OnInit {
  ...
  getProduct() {
    this.dataService.getHttp('core-api/Product/getAvailProduct?outletID=1', '').subscribe((response: any) => {
  ...
  save(item: any){
    ...
    formFields[2].value = item.salePrice;
